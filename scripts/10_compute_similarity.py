...
def main() -> None:
    ...
    signatures = pd.read_parquet(config.processed_path("minhash_signatures.parquet"))
    ...
    pairs.to_parquet(config.processed_path("pairwise_similarity.parquet"), index=False)
    summary = (
    ...
    summary.to_parquet(config.processed_path("similarity_summary.parquet"), index=False)
